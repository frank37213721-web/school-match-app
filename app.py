...
if not st.session_state.logged_in:
    ...
    # 確保選單名稱與下方的 elif 判斷完全一致
    menu = ["課程大廳", "管理中心 (我的課程)", "新增/修改課程", "登出"]
...
if choice == "課程大廳":
    ...
    auth_mode = st.radio("請選擇操作", ["登入", "註冊帳號"], horizontal=True)
    ...
    if auth_mode == "登入":
        ...
        user_identity = st.selectbox("4. 註冊者身份", ["老師", "行政人員", "教務主任"])
        ...
        if st.button("確認註冊"):
            if pwd != confirm_pwd:
                ...
                try:
                    ...
                    st.success(f"🎉 註冊成功！歡迎 {selected_school} 的 {user_name} 老師。")
                    st.balloons()
                except Exception as e:
                    st.error(f"註冊失敗：{e}")
